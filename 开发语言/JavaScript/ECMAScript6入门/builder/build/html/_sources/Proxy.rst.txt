...
.. note:: 注意，要使得 ``Proxy`` 起作用，必须针对 ``Proxy`` 实例（上例是 ``proxy`` 对象）进行操作，而不是针对目标对象（上例是空对象）进行操作。
...
.. code-block:: js
    ...
    pipe(3).double.pow.reverseInt.get; // 63，pipe返回函数，pipe(3)返回代理对象
...
.. note:: 目标对象自身的不可配置（ ``configurable`` ）的属性，不能被 ``deleteProperty`` 方法删除，否则报错。
...
.. note:: 如果目标对象不可扩展（ ``extensible`` ），则 ``defineProperty`` 不能增加目标对象上不存在的属性，否则会报错。另外，如果目标对象的某个属性不可写（ ``writable`` ）或不可配置（ ``configurable`` ），则 ``defineProperty`` 方法不得改变这两个设置。
...
.. note:: ``getPrototypeOf`` 方法的返回值必须是对象或者 ``null`` ，否则报错。另外，如果目标对象不可扩展（ ``extensible`` ）， ``getPrototypeOf`` 方法必须返回目标对象的原型对象。
...
.. note:: 注意，使用Object.keys方法时，有三类属性会被ownKeys方法自动过滤，不会返回。
 - 目标对象上不存在的属性
 - 属性名为 Symbol 值
 - 不可遍历（enumerable）的属性
...
.. note:: 该方法只能返回布尔值，否则会被自动转为布尔值。另外，如果目标对象不可扩展（ ``extensible`` ）， ``setPrototypeOf`` 方法不得改变目标对象的原型。
