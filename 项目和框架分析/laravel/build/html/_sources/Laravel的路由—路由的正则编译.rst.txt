...
.. code-block:: php
    ...
    protected function sendRequestThroughRouter($request)
    {
        ...
        return (new Pipeline($this->app))
                    ->send($request)
                    ->through($this->app->shouldSkipMiddleware() ? [] : $this->middleware) // 这里是全局中间件
...
.. code-block:: php
    ...
    {
        return $this->runRoute($request, $this->findRoute($request)); // 进入路由项正则编译
    }
...
.. code-block:: php
    ...
    {
        $routes = $this->get($request->getMethod()); // 获取请求方法对应的所有路由项

        /**
         * 首先，我们将看看是否可以为此当前请求方法找到匹配的路由。
         * 如果我们可以，那么，我们可以返回它，以便消费者可以调用它。 否则我们将检查另一个动词的路线。
         */
...
.. note:: ``preg_match_all`` 函数用于进行正则表达式全局匹配，成功返回整个模式匹配的次数（可能为零），如果出错返回 ``FALSE`` 。
...
为了减少贪婪型正则表达式的回溯导致的性能浪费，当后续字符串已经结束或者不存在 ``/{x}{y}`` 这样情况的时候，程序将贪婪型正则表达式改为侵占型正则表达式。有关正则表达式的模式请查看： `正则表达式之 贪婪与非贪婪模式详解（概述） <http://blog.csdn.net/u014762221/article/details/68953155>`_
