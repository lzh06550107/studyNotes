...
- ``currentPageResolver`` 为 ``Paginator`` 获取了当前的页码。
    ...
    protected function loadViewsFrom($path, $namespace)
    {
        if (is_array($this->app->config['view']['paths'])) {
            foreach ($this->app->config['view']['paths'] as $viewPath) {
                if (is_dir($appPath = $viewPath.'/vendor/'.$namespace)) {//存在该目录，则添加到视图查询路径中
                    $this->app['view']->addNamespace($namespace, $appPath);
                }
            }
        }
        // 前面目录如果存在，则加载的优先级高于本路径
        $this->app['view']->addNamespace($namespace, $path);
    }

服务的启动函数为分页服务设置了默认的前端分页资源。同时，如果用户自定义了模板文件，则会优先加载该文件。
...
.. code-block:: php
    ...
    {
        $page = $page ?: Paginator::resolveCurrentPage($pageName); // 获取当前的页码

        $total = $this->getCountForPagination($columns); // 获取总的记录数

        $results = $total ? $this->forPage($page, $perPage)->get($columns) : collect(); // 获取当前页的记录集

        return $this->paginator($results, $total, $perPage, $page, [
            'path' => Paginator::resolveCurrentPath(), // 获取当前的请求路径
            'pageName' => $pageName, // 当前分页的名称
        ]);
    }
...
.. code-block:: php
    ...
    <?php
    DB::table('user')->select('*')->where('status',1)->paginate();
