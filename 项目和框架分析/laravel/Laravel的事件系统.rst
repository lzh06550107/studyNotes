...
.. note:: ``Slack`` 是一款即时通信软件，类似于 ``QQ`` ，它提供开放的 ``API`` ，可以调用它向自己团队中指定的个人或者频道（ ``Channel`` )发送消息，因此用它来进行异常通知是再合适不过的。
...
.. note:: 你的事件监听器也可以在构造函数中加入任何依赖关系的类型提示。所有的事件监听器都是通过 Laravel 的 服务容器 来解析的，因此所有的依赖都将会被自动注入。
...
手动访问队列
------------
如果你需要手动访问监听器所在的队列任务的 ``delete`` 和 ``release`` 方法，你可以添加 ``Illuminate\Queue\InteractsWithQueue trait`` 来实现。这个 ``trait`` 会默认加载到生成的监听器中，并提供对这些方法的访问：
